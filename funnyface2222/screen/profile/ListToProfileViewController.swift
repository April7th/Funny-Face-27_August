    ...
    private func setupUI() {
        ...
        for index in 0...9 {
            APIService.shared.GetListImages(albuum:"\(index + 1)"){ response,error in
                self.listData.append(contentsOf: response ?? [ListVideoModal()])
            }
...
//        }
    ...
    @IBAction func clickToImage(_ sender: Any) {
        if let parentVC = findParentViewController(of: UIViewController.self) {
            let storyboard = UIStoryboard(name: "UserImageController", bundle: nil)
            let vc = storyboard.instantiateViewController(withIdentifier: "UserImageController") as! UserImageController
