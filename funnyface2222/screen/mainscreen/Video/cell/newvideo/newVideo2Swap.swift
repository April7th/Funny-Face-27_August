...
//  funnyface2222
//
//  Created by Lê Duy Tân on 19/9/24.
//
...
class newVideo2Swap: UIViewController {
...
//        }
    ...
    @objc func Send_OLD_Images_Click(notification: NSNotification) {
        if let imageLink = notification.userInfo?["image"] as? String {
            ...
            let removeSuot = self.linkImageVideoSwap.replacingOccurrences(of: "\"", with: "", options: .literal, range: nil)
            let linkImagePro = removeSuot.replacingOccurrences(of: "https://photo.gachmen.org", with: "/var/www/build_futurelove", options: .literal, range: nil)
...
extension newVideo2Swap: TrailerPlayerPlaybackDelegate {
...
extension newVideo2Swap: ControlPanelDelegate {
...
extension newVideo2Swap: ReplayPanelDelegate {
...
extension newVideo2Swap : UIPickerViewDelegate,
