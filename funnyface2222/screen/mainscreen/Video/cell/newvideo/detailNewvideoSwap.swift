...
class detailNewvideoSwap: UIViewController {
   ...
   // private let autoReplay = true
    ...
    @IBAction func shareLinkVideoSwap(){
        var linkUrlShare = "https://photo.gachmen.org/detailVideo/" + String(itemDataSend.ip_tao_vid ?? "")
...
extension detailNewvideoSwap: TrailerPlayerPlaybackDelegate {
    ...
    func trailerPlayer(_ player: TrailerPlayer, didUpdatePlaybackTime time: TimeInterval) {
            if player == playerView.player {
                ...
                // Check if the video has reached the end based on playback time and duration
                if time >= playerView.duration {
                    hasVideoEnded = true
                } else {
                    hasVideoEnded = false
                }
            ...
            if player == playerGocView.player {
                ...
                // Check if the video has reached the end based on playback time and duration
                if time >= playerGocView.duration {
                    hasVideoEnded = true
                } else {
                    hasVideoEnded = false
                }
...
extension detailNewvideoSwap: ReplayPanelDelegate {
    ...
    func replayPanel(_ panel: ReplayPanel, didTapReplayButton button: UIButton) {
            ...
            guard !hasVideoEnded else {
                // Tùy chọn hiển thị thông báo cho biết video đã kết thúc
