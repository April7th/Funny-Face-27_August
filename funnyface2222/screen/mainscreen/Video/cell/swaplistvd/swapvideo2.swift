...
//    @IBOutlet weak var percentLabel: UILabel!
    ...
    @IBOutlet weak var circleAddImage: UIImageView!
        ...
        if oldURLBool == false{
            ...
            for x in 0..<100 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + Double(x)+0.25, execute: {
                    ...
                    self.progressCountLabel.text = "\(self.countProgress)% processing progress, please wait! "
                })
            }
...
//        timerLabel.text = dateComponentsFormatter.string(from: components)
    ...
    @objc func handleImageNotification(_ notification: NSNotification) {
        if let userInfo = notification.userInfo, let imageLink = userInfo["image"] as? String {
            ...
            let linkImagePro = imageLink.replacingOccurrences(of: "https://photo.gachmen.org", with: "/var/www/build_futurelove", options: .literal, range: nil)
            self.linkImageVideoSwap = linkImagePro
            self.oldURLBool = true
            ...
            self.boyImage.kf.setImage(
                ...
                case .success(let value):
                    ...
                    self.circleAddImage.isHidden = true
...
//    }()
    ...
    override func viewDidLoad() {
        ...
        progressView.trackTintColor = .white
        progressView.progressTintColor = UIColor(hex: "#1DB954")
        
        startButton.titleLabel?.font = .quickSandBold(size: 14)
...
//        circularSlider.endPointValue = 0
        ...
        playerView.heightAnchor.constraint(equalTo: playerView.widthAnchor, multiplier: 0.6).isActive = true
