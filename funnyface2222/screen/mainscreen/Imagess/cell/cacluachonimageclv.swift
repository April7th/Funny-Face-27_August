...
extension cacluachonimageclv: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let parentVC = findParentViewController(of: UIViewController.self) {
            let nextViewController = ImageHighlightUIViewController(nibName: "ImageHighlightUIViewController", bundle: nil)
//            nextViewController.imageView = self.
            let selectedItem = self.listData[indexPath.row]
            nextViewController.dataList = "successfulldasdklsahlasldla"
            nextViewController.dataList = "Test"
            
            let url = URL(string: self.listData[indexPath.row].image ?? "")
            
            
            nextViewController.urlString = url.asIntOrEmpty()
//            let processor = DownsamplingImageProcessor(size: nextViewController.ima.bounds.size)
